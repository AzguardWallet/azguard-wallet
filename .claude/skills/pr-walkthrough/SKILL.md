---
name: pr-walkthrough
description: Interactive guided tour through a PR or diff — explains the changes file-by-file so a human can understand them before forming an opinion. Use when the user wants to UNDERSTAND a change — "walk me through PR <n>", "explain PR <n>", "show me PR <n>", "what's in this PR", "go over the changes", "break down PR <n>", "что в этом PR". This skill is for understanding, not judgment: when the user asks to review code quality, find bugs, or assess whether something is ready to merge ("review this PR", "code review", "проверь качество"), use the code-review or quality-assessment skill instead.
---
