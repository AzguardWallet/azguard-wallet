		...
		try {
			...
			for (let i = 0; i < tbs.length; i++) {
				...
				if (token.balanceOfPrivateFn) {
					const balanceOfPrivateFn = BalanceOfPrivateFn.new(token.balanceOfPrivateFn.name, token.balanceOfPrivateFn.impl)
				...
				if (token.balanceOfPublicFn) {
					const balanceOfPublicFn = BalanceOfPublicFn.new(token.balanceOfPublicFn.name, token.balanceOfPublicFn.impl)
