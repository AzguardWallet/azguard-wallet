...
test("mint token via faucet", { timeout: 360_000 }, async ({ registeredExtension }) => {
	...
	// 3. Click faucet option → faucet form
	// NOTE: The "Azguard Faucet" text may come from dynamic data — update selector if UI changes after rebrand
	...
	await page.waitForFunction(() => document.body.innerText.includes("Minting in progress"), { timeout: 30_000 })

	// 5. Wait for deploy tx — "Latest transaction" section appears
	await page.waitForFunction(() => document.body.innerText.includes("Latest transaction"), { timeout: 300_000 })

	// 6. Wait for minting to fully complete (no more "Minting" text)
	await page.waitForFunction(
		() => !document.body.innerText.includes("Minting in progress") && !document.body.innerText.includes("Minting more tokens"),
		{ timeout: 180_000 },
	)
	...
	await page.evaluate(() => {
		const tokensHeader = [...document.querySelectorAll("*")].find((el) => el.textContent?.trim() === "Tokens")
		if (tokensHeader) {
	...
	await page.waitForFunction(() => !document.body.innerText.includes("Refreshing balance"), { timeout: 60_000 })

	// 9. Verify balance = 100 TST
	await page.waitForSelector("text/Test Token", { visible: true })
	await page.waitForFunction(() => document.body.innerText.includes("100"), { timeout: 10_000 })
	...
	await page.evaluate(() => {
		...
		const candidates = allElements.filter(
			(el) => el.textContent?.includes("Test Token") && el.textContent?.includes("Use token balance"),
		)
