import { AztecAddress, deriveKeys, Fr, getContractInstanceFromDeployParams, GrumpkinScalar } from "@aztec/aztec.js";
...
export class SchnorrAccountContractV0 implements IAccountContract {
    ...
    public async getAddress(): Promise<AztecAddress> {
        const secret = await this.profile.deriveChildSecret(this.network.chainId, this.account);
        const contract = new SchnorrAccountContract(this._deriveSigningKey(secret));
