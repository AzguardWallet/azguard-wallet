...
export class ExecutionService extends Service<Methods> implements ServiceSpec<Methods> {
	...
	public async executeTransfer(
		...
		try {
			...
			const [txRequest, node, pxe, account, network, nonce, __, feePaymentMethod] = await this.buildAndEstimateTxRequest(
				op,
				op.feeSettings,
				transferTask,
			)
	...
	private async executeRegisterContract(op: RegisterContractOperation): Promise<void> {
		...
		const instance = providedInstance ?? (await this.pxeService.getContractInstance(network, AztecAddress.fromString(op.address)))
		if (!instance) {
			throw new Error("Contract instance not found")
		}

		const providedArtifact = await ContractArtifactSchema.optional().parseAsync(op.artifact)
		const artifact = providedArtifact ?? (await this.pxeService.getContractArtifact(network, instance.currentContractClassId))
		if (!artifact) {
	...
	private async executeNoFromSendTx(
		...
		const [txRequest, node, pxe, account, network, txCalls] = await this.buildNoFromTxRequest(op, parentTask)
		this.logDebug(
		...
		const provedTx = await this.proveTxTask(pxe, txRequest, scopesWithAccount, parentTask)
		const timestamp = provedTx.publicInputs.constants.anchorBlockHeader.globalVariables.timestamp
		const offchainOutput = extractOffchainOutput(
			provedTx.getOffchainEffects(),
			typeof timestamp.toBigInt === "function" ? timestamp.toBigInt() : BigInt(timestamp.toString()),
	...
	private async buildNoFromTxRequest(
		...
		try {
			...
			for (const [contract, instance] of instances) {
				if (!registeredContracts.has(contract)) {
					this.logDebug(
						`buildNoFromTxRequest: registering contract ${contract} with classId ${instance.currentContractClassId.toString()}`,
					)
					await pxe.registerContract({
						instance,
						artifact: artifacts.get(instance.currentContractClassId.toString()),
	 ...
	 */
	private async discoverRequiredAuthWitnesses(op: {
		networkId: string
		accountAddress: string
		actions: Action[]
	}): Promise<AddPrivateAuthwitAction[]> {
		const [txRequest, node, pxe, account] = await this.buildTxRequest(op, AzguardFeePaymentMethod.FeeJuice)
	...
	private async buildAndEstimateTxRequest(
		...
		feeSettings: FeeSettings,
		parentTask?: WrappedTask,
	): Promise<[TxExecutionRequest, AztecNode, IPXE, IAccountContract, Network, Fr, TxCall[], AzguardFeePaymentMethod]> {
		...
		try {
			...
			switch (feePaymentMethod.kind) {
				...
				case "fjwc": {
					...
					await this.finalizeGasLimits(node, txRequest, simulatedTx, gasPadding, undefined, op.fee, feeMultiplier)
					task.complete()
					return [txRequest, node, pxe, account, network, nonce, txCalls, AzguardFeePaymentMethod.FeeJuiceWithClaim]
				}
				case "fpc": {
				...
				switch (action.kind) {
					...
					case "add_private_authwit": {
						...
						switch (action.content.kind) {
							...
							case "encoded_call": {
								messageHash = await this.getEncodedCallMessageHash(action.content, nodeInfo, instances, artifacts)
								break
							}
							case "intent": {
	...
	private async simulateTxTask(pxe: IPXE, txRequest: TxExecutionRequest, opts: SimulateTxOpts, parentTask?: WrappedTask) {
	...
	private async getArtifacts(pxe: IPXE, instances: Map<string, ContractInstanceWithAddress>): Promise<Map<string, ContractArtifact>> {
