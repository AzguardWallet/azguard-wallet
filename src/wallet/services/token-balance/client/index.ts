...
import { TokenBalanceServiceEvent, TokenBalanceServiceEventMessage } from "./events";
...
export class TokenBalanceServiceClient extends ServiceClient {
    ...
    constructor(
        onConnected?: () => void,
        onDisconnected?: () => void,
        private readonly onTokenBalanceAdded?: (token: TokenBalanceInfo) => void,
        private readonly onTokenBalanceUpdated?: (token: TokenBalanceInfo) => void,
        private readonly onTokenBalanceDeleted?: (token: TokenBalanceInfo) => void,
    ) {
        ...
        switch (message.event) {
            case TokenBalanceServiceEvent.TokenBalanceAdded:
                if (this.onTokenBalanceAdded) {
                    try {this.onTokenBalanceAdded((message as TokenBalanceServiceEventMessage).tokenBalance);}
                    catch {}
                }
                break;
            case TokenBalanceServiceEvent.TokenBalanceUpdated:
                if (this.onTokenBalanceUpdated) {
                    try {this.onTokenBalanceUpdated((message as TokenBalanceServiceEventMessage).tokenBalance);}
                    catch {}
                }
                break;
            case TokenBalanceServiceEvent.TokenBalanceDeleted:
                if (this.onTokenBalanceDeleted) {
                    try {this.onTokenBalanceDeleted((message as TokenBalanceServiceEventMessage).tokenBalance);}
                    catch {}
                }
                break;
            default:
     ...
     */
    public getTokenBalances(token?: number, account?: string): Promise<TokenBalanceInfo[]> {
