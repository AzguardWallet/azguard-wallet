...
export class TokenBalanceService extends Service {
    ...
    public async process(request: RequestMessage): Promise<ResponseMessage | undefined> {
        switch(request.method) {
            case TokenBalanceServiceMethod.GetTokenBalances: {
                ...
                try {
                    const balances = await this.getBalances(_request.token, _request.account);
                    return new GetTokenBalancesResponse(_request, balances);
                }
                catch (error: any) {
                    return new GetTokenBalancesResponse(_request, undefined, error.message);
                }
            }
            case TokenBalanceServiceMethod.RefreshTokenBalance: {
                ...
                try {
                    ...
                    return new RefreshTokenBalanceResponse(_request, error.message);
                }
            }
            default: {
    ...
    public async getBalances(tokenId?: number, accountAddress?: string): Promise<TokenBalanceInfo[]> {
    ...
    private async createTokenBalance(token: Token, account: Account) {
        ...
        this.emit(new TokenBalanceServiceEventMessage(TokenBalanceServiceEvent.TokenBalanceAdded, this.getTokenBalanceInfo(tb)));
    ...
    private readonly onTokenAdded = async (token: Token) => {
        ...
        for (const account of await this.accountService.getAccounts(this.profile!, token.chainId, true)) {
            await this.createTokenBalance(token, account);
        }
    }
    ...
    private async init() {
        while(true) {
            try {
                this.profile = (await this.profileService.readActiveProfile())?.id;
                    ...
                    if (this.queue.length) {
                        ...
                        while (this.queue.length) {
                            await Promise.allSettled(
                                this.queue.dequeueBatch(4).map(x => this.syncTokenBalance(x, cache)) // TODO: settings
                            );
                        }
                        const end = Date.now();
                        console.debug(`Token balances synced in ${end - start}ms`);
                    }
                }
                catch (error) {
    ...
    private async syncTokenBalance(tb: TokenBalanceRaw, cache: Map<string, IAccountContract>) {
        try {
            ...
            if (!account) {
                account = await this.accountService.getAccountContract(this.profile!, token.chainId, tb.account);
            ...
            if (privateBalance !== tb.privateBalance || publicBalance != tb.publicBalance) {
                console.debug(`Balance #${tb.id} changed: `, privateBalance, publicBalance);
                tb.privateBalance = privateBalance;
                tb.publicBalance = publicBalance;
                tb.updatedAt = Date.now();
                await this.balances.set(`${tb.id}`, tb);
                this.emit(new TokenBalanceServiceEventMessage(TokenBalanceServiceEvent.TokenBalanceUpdated, this.getTokenBalanceInfo(tb)));
            }
            else {
