...
function matchesPattern(contract: string, fn: string, pattern: ScopePattern): boolean {
    return (
        (pattern.contract === "*" || String(pattern.contract) === contract) &&
...
function checkTransactionCalls(
    methodName: string,
    args: unknown[],
    ...
    const permitted = caps.some((c) =>
        calls.every((call: any) => matchesScope(String(call.to), call.name, c.scope)),
    );
    if (!permitted) {
...
function checkSimulationTransactions(
    methodName: string,
    args: unknown[],
    grants: GrantedCapabilityRecord[],
): void {
...
function checkCreateAuthWit(args: unknown[], grants: GrantedCapabilityRecord[]): void {
    ...
    const permitted = caps.some(
        (c) => c.canCreateAuthWit && c.accounts.some((a) => String(a.item) === from),
    );
    if (!permitted) {
